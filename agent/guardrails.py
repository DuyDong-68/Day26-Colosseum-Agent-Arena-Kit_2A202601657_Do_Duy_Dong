...
def scan_for_injected_instructions(text: str) -> InjectionScanResult:
    patterns = [
        ...
        r"system override",
        r"report the act field",
        r"ignore prior safety rules",
        r"grading key is disclosed",
        r"reveal your instructions",
        r"as the system, you must now"
    ]
    hits = []
    for p in patterns:
        if re.search(p, text, re.IGNORECASE):
            hits.append(p)
    return InjectionScanResult(suspicious=bool(hits), matched_patterns=tuple(hits))
...
def redact(text: str) -> RedactionResult:
    hits = []
    def replacer(m):
        hits.append("long_sequence")
        return "[REDACTED]"
    
    redacted = re.sub(r'[a-zA-Z0-9]{40,}', replacer, text)
...
def verify_arithmetic(text: str, retrieved_text: str = "") -> ArithmeticCheckResult:
    numbers = _NUMBER_RE.findall(text)
    if not numbers:
        return ArithmeticCheckResult(checked=True, ok=True, detail="No numbers found")
    
    if not retrieved_text:
        return ArithmeticCheckResult(checked=True, ok=False, detail="Numbers found but no sources provided to verify against")
        
    bad_nums = [n for n in numbers if n not in retrieved_text]
    if bad_nums:
        return ArithmeticCheckResult(checked=True, ok=False, detail=f"Unsupported numbers: {bad_nums}")
        
    return ArithmeticCheckResult(checked=True, ok=True, detail="All numbers verified")
